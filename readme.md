...
| 风格控制 | 8 大类、50+ 种整体风格，支持情绪、语调、音色、方言、角色等 |
| 细粒度标签 | 插入呼吸、停顿、颤抖、哭笑等精细控制标签 |
| 个人 API Key | 每位用户可输入自己的 API Key，互不影响 |
| 热加载配置 | 通过网页实时修改 API Key/API URL，无需重启服务 |
...
| 浏览器 | 上传文件 | 浏览器录音 | 说明 |
| --- | --- | --- | --- |
| Chrome / Edge（桌面） | 支持 | 支持 | 需 HTTPS |
| Safari（macOS） | 支持 | 支持 | 需 HTTPS |
| Safari（iOS） | 支持 | 支持 | iOS 唯一完全支持录音的浏览器 |
| Chrome（Android） | 支持 | 支持 | 需 HTTPS |
| Chrome（iOS） | 支持 | 可能受限 | iOS 上 Chrome 使用 WebKit 引擎，可能存在兼容性问题，建议用 Safari |
| Firefox（桌面） | 支持 | 支持 | 需 HTTPS |
| 微信内置浏览器 | 支持 | 不支持 | 不支持 MediaRecorder，仅可上传文件 |
| Safari（iOS < 14.5） | 支持 | 不支持 | 系统版本过低，不支持 MediaRecorder |

> 重要：浏览器录音功能（getUserMedia）要求 HTTPS 连接。HTTP 下无法使用录音。
...
mimo-clone/
├── app.py                 # Flask 后端主程序
├── config.json            # 应用配置文件（端口、API Key、模型等）
├── hot_config.json        # 热加载配置文件（网页端动态修改）
├── mimo.sh                # 一体化管理脚本（部署/启动/停止/卸载）
├── watchdog.sh            # 守护进程脚本（自动生成）
├── requirements.txt       # Python 依赖
├── passenger_wsgi.py      # Passenger WSGI 入口（部分面板使用）
├── templates/
│   └── index.html         # 前端页面（HTML + CSS + JS 全内联）
├── static/
│   └── css/
│       └── style.css      # 全局样式
├── uploads/               # 用户上传的参考音频（临时存储）
├── outputs/               # 克隆生成的音频文件
├── venv/                  # Python 虚拟环境
├── app.log                # 应用运行日志
└── watchdog.log           # 守护进程日志
...
- 一个有效的 MiMo TTS API Key，可从 [mimo.xiaomi.com](https://mimo.xiaomi.com) 获取
...
本项目使用 Python 虚拟环境（venv）隔离依赖，避免污染系统 Python 环境。

> `mimo.sh deploy` 会自动完成虚拟环境创建和依赖安装。以下内容供手动部署或理解原理时参考。

### 虚拟环境路径

```text
~/mimo-clone/venv/              # 虚拟环境目录
~/mimo-clone/venv/bin/python3   # 虚拟环境中的 Python
~/mimo-clone/venv/bin/pip       # 虚拟环境中的 pip
```
...
# 激活后终端会显示 (venv) 前缀，表示已进入虚拟环境
# (venv) root@server:~/mimo-clone#
...
#### 系统提示 `python: not found` 或 `python3: not found`

Alpine Linux 默认可能未安装 Python：
...
# 方式二：直接使用虚拟环境的 Python 绝对路径
~/mimo-clone/venv/bin/python3 app.py
```

不要用 `python app.py` 运行。建议使用 `python3`，并确保已激活虚拟环境或使用虚拟环境中的绝对路径。

#### 虚拟环境损坏或丢失

```bash
# 删除旧的虚拟环境
rm -rf ~/mimo-clone/venv
...
推荐使用 `./mimo.sh start` 代替手动启动，因为它会同时启动 watchdog 守护进程。

### 在后台脚本中使用虚拟环境

在 `nohup` 或 `cron` 等非交互式场景中，`source venv/bin/activate` 可能不生效。建议直接使用虚拟环境的 Python 绝对路径：
...
# 推荐：直接使用绝对路径
/root/mimo-clone/venv/bin/python3 /root/mimo-clone/app.py
```

这也是 `watchdog.sh` 采用的写法。
...
```bash
./mimo.sh deploy    # 首次部署（检查环境 + 安装依赖 + 配置）
...
| 机制 | 说明 |
| --- | --- |
| 健康检查间隔 | 每 30 秒检测一次 `app.py` 是否存活 |
| 自动拉起 | `app.py` 崩溃或被终止后，30 秒内自动重启 |
| 开机自启 | 系统重启后 watchdog 自动启动（通过 `~/.profile`） |
| 日志记录 | 所有拉起事件记录在 `watchdog.log` |
...
不要选择 Full 或 Full (Strict)，因为源站没有配置 SSL 证书，选择 Full 可能导致 502/522 错误。
...
如果你的端口不在以上列表中，例如 `30619`，需要创建 Origin Rule：
...
访问 [test-ipv6.com](https://test-ipv6.com)。如果显示 IPv6 地址则支持。
...
通过网页顶部的 API 配置面板可以实时修改 API Key 和 API URL，无需重启服务。
...
- 使用 Safari 浏览器访问，推荐此方式
- 或使用“上传文件”功能

### 3. 克隆后提示 `API请求失败: 400 - Unsupported audio.voice source format`

原因：上传的音频格式不被 API 支持，例如原始 webm/m4a 格式。
...
### 4. 克隆后提示“网络错误”或 `Failed to fetch`
...
- 如果直连正常，问题可能是 Cloudflare 超时，可缩短 API 超时时间
...
NAT VPS 的 `free -m` 显示的是宿主机内存，不一定代表你的 VPS 实际占用。你的 VPS 进程通常只占用约 20-50MB 内存，正常使用无需担心。

如果确实内存紧张，重启 VPS 即可释放，watchdog 会自动在重启后拉起应用。
...
- 访问认证建议：如有需要，可在 Cloudflare 层面添加 Access 认证，限制访问人群
...
本项目仅供学习和技术研究使用。请确保遵守相关法律法规，不得将声音克隆技术用于欺诈、冒充他人等非法用途。
